...
class wpAppbox_CreateOutput {
	...
	*/
	...
	function returnImageProxyURL( $imageURL ) {
		if ( !get_option('wpAppbox_imgProxy') ) return( $imageURL );
		...
		$imageURL = plugins_url() . '/wp-appbox/wp-appbox.php?imageurl=' . $imageURL;
		return( $imageURL );
	}
	...
	*/
	...
	function returnAppLink( $appLink, $appID ) {
		...
		*/
		if ( preg_match( '/(:https?\:\/\/)?(www.)microsoft.com/i', $appLink ) ):
			if ( get_option('wpAppbox_userAffiliate') ):
				$authorID = get_the_author_meta('ID');
			...
			if ( ( '' != $affiliateID ) && ( '' != $programID ) ):
				$creativeID = '0';
				foreach( $wpAppbox_MicrosoftPrivateAffiliateProgramm as $tempKey1 => $tempArr1 ):
					if ( $programID == $tempArr1[0] ) {
						$creativeID = $tempArr1[1];
						continue;
					}
				endforeach;
		...
		switch ( $storeID ):
			case 'appstore':
				...
				case 'appletv':
					...
					break;
				default:
					$appScreenshots = array_merge( $appScreenshots['iphone'], $appScreenshots['ipad'], $appScreenshots['imessage'], $appScreenshots['watch'], $appScreenshots['appletv'] );
					break;
				endswitch;
				break;
			case 'windowsstore':
				...
				case 'desktop':
					...
					break;
				case 'xbox':
					$appScreenshots = $appScreenshots['xbox'];
					break;
				default:
					$appScreenshots = array_merge( $appScreenshots['mobile'], $appScreenshots['desktop'], $appScreenshots['xbox'] );
					break;
				endswitch;
				break;
		endswitch;
	...
	*/
	...
	function theOutput( $attr ) {
		...
		*/
		...
		$template = str_replace( '{ICON}', $this->returnAppIcon( $appData['id'], $appData['app_icon'], $appData['store_name_css'], ( $attr['store'] == 'windowsstore' && isset( $appData['app_extend']['windowsstorebg'] )? $appData['app_extend']['windowsstorebg'] : '' ) ), $template );
