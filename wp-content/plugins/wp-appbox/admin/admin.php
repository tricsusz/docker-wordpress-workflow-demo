...
function wpAppbox_showAdminNotification() {
	if ( ( WPAPPBOX_PLUGIN_VERSION != get_option( 'wpAppbox_notifyLastV' ) ) && ( 'abc' == '123' ) ):
		?>
...
function wpAppbox_saveSettings() {
	...
	switch ( $tab ) {			
		case 'output':
	    	update_option( 'wpAppbox_downloadCaption', ( !empty( $_POST['wpAppbox_downloadCaption'] ) ? sanitize_text_field( $_POST['wpAppbox_downloadCaption'] ) : $wpAppbox_optionsDefault['downloadCaption'] ), 'no' );
		...
		case 'cache':
			...
			update_option( 'wpAppbox_cacheMode', ( !empty( $_POST['wpAppbox_cacheMode'] ) != '' ? sanitize_text_field( $_POST['wpAppbox_cacheMode'] ) : $wpAppbox_optionsDefault['cacheMode'] ), 'no' );
			...
			if ( isset( $_POST['wpAppbox_imgCacheMode'] ) && is_array( $_POST['wpAppbox_imgCacheMode'] ) ) {
				update_option( 'wpAppbox_imgCacheMode',  array_map( 'strip_tags', $_POST['wpAppbox_imgCacheMode'] ), 'no');
			}
			else {
    	...
    	case 'advanced':
	    	...
	    	update_option( 'wpAppbox_amaAPIsecretKey', ( !empty( $_POST['wpAppbox_amaAPIsecretKey'] ) ? base64_encode( strip_tags( trim( $_POST['wpAppbox_amaAPIsecretKey'] ) ) ) : '' ), 'no' );
	    	update_option( 'wpAppbox_amaAPIpublicKey', ( !empty( $_POST['wpAppbox_amaAPIpublicKey'] ) ? sanitize_text_field( trim( $_POST['wpAppbox_amaAPIpublicKey'] ) ) : '' ), 'no' );
	    	...
	    	update_option( 'wpAppbox_amaAPIregion', ( !empty( $_POST['wpAppbox_amaAPIregion'] ) ? sanitize_text_field( trim( $_POST['wpAppbox_amaAPIregion'] ) ) : '' ), 'no' );
    		...
    		update_option( 'wpAppbox_eOutput', ( !empty( $_POST['wpAppbox_eOutput'] ) ? sanitize_text_field( trim( $_POST['wpAppbox_eOutput'] ) ) : $wpAppbox_optionsDefault['eOutput'] ), 'no' );
