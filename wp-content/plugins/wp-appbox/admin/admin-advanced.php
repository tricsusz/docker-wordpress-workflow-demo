...
<table class="form-table">
	...
	<tr valign="top" class="amaAPItr" <?php if( true != get_option( 'wpAppbox_amaAPIuse' ) ): ?> style="display:none;"<?php endif; ?>>
		...
		<td>	
			<input type="password" style="width: 400px;" name="wpAppbox_amaAPIsecretKey" id="wpAppbox_amaAPIsecretKey" value="<?php echo( base64_decode( get_option('wpAppbox_amaAPIsecretKey') ) ); ?>" />
		</td>
	</tr>
...
<table class="form-table">
	...
	<?php if ( !wpAppbox_imageCache::checkImageCache() || 3 != count( get_option('wpAppbox_imgCacheMode') ) ): ?>
	<tr valign="top">
		<th scope="row"><label for="wpAppbox_imgProxy"><?php esc_html_e( 'Image proxy', 'wp-appbox' ); ?>:</label></th>
		<td>	
			<label for="wpAppbox_imgProxy">
				...
				<?php esc_html_e( 'Retrieves app icons and screenshots using its own image proxy. Warning: Needs much more resources.', 'wp-appbox' ); ?> <?php esc_html_e( 'Compliant with GDPR.', 'wp-appbox' ); ?>
			</label>
		</td>
	</tr>
	<?php endif; ?>
	...
	<tr valign="top">
		...
		<td>	
			<label for="wpAppbox_renderGutenberg">
				...
				<?php esc_html_e( 'Render the output of the appboxes in Gutenberg as a preview', 'wp-appbox' ); ?>
			</label>
		</td>
	</tr>
	<?php endif; ?>
	...
	<tr valign="top">
		...
		<td>	
			...
			<div style="display:none;margin-top:8px;" id="wpAppbox_resetCache">
				<a class="button action" style="margin-right: 10px"; href="/wp-admin/options-general.php?page=wp-appbox&tab=<?php echo( $_GET['tab'] ); ?>&flushcache" onClick="return confirm('<?php esc_html_e( 'Are you sure that the cache should be cleared? All data must be reloaded from the server of the operator.', 'wp-appbox' ); ?>')"><?php esc_html_e( 'Clear cache', 'wp-appbox' ); ?></a>
				<?php if ( wpAppbox_imageCache::quickcheckImageCache() ): ?><a class="button action" style="margin-right: 10px"; href="/wp-admin/options-general.php?page=wp-appbox&tab=<?php echo( $_GET['tab'] ); ?>&flushimgcache" onClick="return confirm('<?php esc_html_e( 'Are you sure that all cached images should be deleted? All images must be reloaded from the server of the operator.', 'wp-appbox' ); ?>')"><?php esc_html_e( 'Clear image cache', 'wp-appbox' ); ?></a><?php endif; ?>
				<a class="button action" style="margin-right: 10px"; href="/wp-admin/options-general.php?page=wp-appbox&tab=<?php echo( $_GET['tab'] ); ?>&resetdeprecated" onClick="return confirm('<?php esc_html_e( 'Are you sure that all deprecated apps should be reset and marked as available?', 'wp-appbox' ); ?>')"><?php esc_html_e( 'Reset deprecated apps', 'wp-appbox' ); ?></a>
				<a class="button action" style="margin-right: 10px"; href="/wp-admin/options-general.php?page=wp-appbox&tab=<?php echo( $_GET['tab'] ); ?>&resetblockedqueries" onClick="return confirm('<?php esc_html_e( 'Are you sure that all blocked queries should be reset?', 'wp-appbox' ); ?>')"><?php esc_html_e( 'Reset blocked queries', 'wp-appbox' ); ?></a>
			</div>
		</td>
	</tr>
