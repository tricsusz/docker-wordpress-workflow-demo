...
class Cached_Apps extends WP_List_Table {
	...
	*/
	...
	function fillData( $search = '', $filter = '' ) {
		...
		$sql = "SELECT * FROM " . wpAppbox_databaseName() . " $sqlSearch $sqlFilter ORDER BY app_title";
	...
	*/
	...
	function prepare_items() {
		...
		if ( $this->isActiveSearch() || $this->isActiveFilter() ) {
			$apps = $this->fillData( $_POST['cacheSearch'], $_POST['cacheStore'] );
		} else {
			$apps = $this->fillData(); 
		}
		   	...
		   	<div class="alignleft actions bulkactions" style="margin-bottom:7px;">
		   	  		...
		   	  		<select id="store" name="cacheStore" style="height:27px;" onChange="this.form.submit()">
		   	  			...
		   	  			<?php
		   	  				...
		   	  				if ( isset( $wpAppbox_storeNames ) ):
		   	  					...
		   	  					?>
		   	  						<option value="<?php echo( $storeID ); ?>" <?php selected( $_POST['cacheStore'], $storeID ); ?>><?php echo( $storeName); ?></option>
		   	  					<?php
		   	  					endforeach;
		   	  				endif;
		   	  			?>
		   	  	...
		   	  	<div name="listSearch" style="float:left;">
			   	  	<label class="screen-reader-text" for="search_id-search-input"><?php _e('Search', 'wp-appbox') ?>:</label> 
			   	   	<input id="search_id-search-input" style="height:27px;" type="text" name="cacheSearch" value="<?php if ( isset( $_POST['cacheSearch'] ) ) echo( esc_html( $_POST['cacheSearch'] ) ); ?>" /> 
