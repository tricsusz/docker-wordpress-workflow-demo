...
experimental_imgProxy => <?php echo( ( get_option('wpAppbox_imgProxy') ? 'true' : 'false') ); ?>&#013;
...
<?php foreach( $wpAppbox_storeNames as $storeID => $storeName ) {
	if ( !in_array( $storeID, $wpAppbox_storeURL_noLanguages ) ) {
		echo( $storeName . '=>' );
		if ( 'amazonapps' == $storeID && get_option('wpAppbox_amaAPIuse') )
			echo( 'Amazon API' ); 
		else
...
<?php } ?>
affiliateAppleID => <?php echo( ( get_option('wpAppbox_affiliateAppleID') ? 'true' : 'false') ); ?>&#013;
