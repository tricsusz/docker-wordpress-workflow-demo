...
function wpAppbox_UpdateAction() {
	...
	if ( wpAppbox_checkOlderVersion( '4.0.63' ) ) {
		global $wpdb; 
		$sql = "DELETE FROM wp_options WHERE option_name LIKE ('%wpAppbox_%affiliateApple%')";
		$wpdb->query( $sql );
		...
		$sql = "DELETE FROM wp_options WHERE option_name LIKE ('%wpAppbox_%firefoxmarketplace%')";
		$wpdb->query( $sql );
	}
...
function wpAppbox_registerGutenbergBlock() {
	...
	wp_register_script(
		'wp-appbox-block',
		plugins_url( 'editor/gutenberg/block.js', __FILE__ ),
